...
const PricingUser = () => {
  ...
  <div className="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
      <h2 className="text-3xl font-extrabold text-center mb-10">Pricing Plans</h2>
      
      <div className="mt-6 grid gap-6 md:grid-cols-2 lg:grid-cols-4">
      {plans.map((plan) => (
  <div key={plan.name} className="border border-gray-300 shadow-lg rounded-lg p-6 hover:shadow-xl transition-shadow duration-300 ease-in-out">
    <div className="flex justify-between items-center">
      <h3 className="text-lg leading-6 font-medium text-gray-900">{plan.name}</h3>
      {plan.savings && <span className="text-sm font-semibold text-green-600">Save ${plan.savings}</span>}
    </div>
    <div className="flex items-baseline">
      <p className="text-4xl font-extrabold text-gray-900 mr-2">${plan.price}</p>
      {plan.originalPrice && <p className="text-lg text-gray-500 line-through">${plan.originalPrice}</p>}
    </div>
    <ul className="mt-4 list-none pl-6 space-y-4 text-sm">
  {plan.features.map((feature, index) => (
    <li key={index} className="flex items-center">
      {feature.enabled ? (
        <><span className="text-black-500 mr-2">⇨</span>{feature.name}</>
      ) : (
        <><span className="text-red-500 mr-2">✕</span>{feature.name}</>
      )}
    </li>
  ))}
</ul>
...
<a href={plan.link} className="mt-6 inline-block w-full text-center text-black font-bold py-3 px-6 rounded-lg border border-transparent hover:border-gray-300 bg-[#dedede] shadow-sm transition-all duration-300 ease-in-out transform hover:-translate-y-1 hover:shadow-md">
  ...
  <div className="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
    <h2 className="text-3xl font-extrabold text-center mb-8">To put things into perspective</h2>
    <div className="mt-6 grid gap-6 md:grid-cols-2 lg:grid-cols-4">
...
<div className="max-w-4xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
  <h2 className="text-2xl font-extrabold text-center text-black mb-6">Let's do a Cost-Benefit Analysis</h2>
    ...
    <div className="space-y-4">
      ...
      {[
        ...
        <div key={index} className="flex flex-col items-center">
          ...
          {index < 5 && <div className="w-1 h-6 bg-gray-300"></div>} {/* Small connectors */}
        </div>
      ))}
    </div>
  </div>
...
function DocumentIcon(props: JSX.IntrinsicAttributes & SVGProps<SVGSVGElement>) {
    return (
      <svg
        {...props}
